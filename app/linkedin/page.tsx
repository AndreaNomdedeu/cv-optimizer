...
function Nav() {
  const links = [["Resume","/resume"],["Cover letters","/cover-letters"],["LinkedIn","/linkedin"],["Pricing","/#pricing"]];
  return (
...
const FEATURES = [
  ["Headline & About rewrite", "We rewrite your headline and About section with recruiter-friendly language and the keywords your target roles actually search for."],
  ["Keyword & SSI boost", "See which skills and keywords are missing from your profile so you rank higher in LinkedIn recruiter searches."],
  ["Profile-to-resume", "Turn your LinkedIn profile into a polished, ATS-ready resume in one click — no retyping."],
  ["Experience polish", "Transform flat job descriptions into achievement-driven bullet points with measurable impact."],
  ["Photo & banner tips", "Get guidance on a professional photo and banner that match your industry and seniority."],
  ["Completeness score", "A clear score that shows exactly what to fix to reach an all-star, recruiter-ready profile."],
];

const STEPS = [
  ["1", "Connect your profile", "Paste your LinkedIn URL or import your existing resume."],
  ["2", "Get your analysis", "We scan every section against recruiter best practices and your target role."],
  ["3", "Apply suggestions", "Accept one-click rewrites and watch your profile score climb."],
];
  ...
  return (
    <div style={{ background: C.surface, minHeight: "100vh" }}>
      ...
      <section className="mx-auto max-w-6xl px-6 pt-16 pb-10 text-center">
        <span className="inline-block rounded-full px-3 py-1 text-xs font-bold" style={{ background: "#EEF0FF", color: C.primary }}>LINKEDIN OPTIMIZATION</span>
        <h1 className="mx-auto mt-5 max-w-3xl text-4xl font-extrabold leading-tight md:text-5xl" style={{ color: C.ink }}>Make recruiters find you on LinkedIn.</h1>
        <p className="mx-auto mt-5 max-w-2xl text-lg" style={{ color: C.inkSoft }}>CV Optimizer audits your LinkedIn profile the way a recruiter would — then rewrites it to rank higher in search and win more interview requests.</p>
        <div className="mt-8 flex justify-center gap-3">
          <Link href="/linkedin/optimize" className="rounded-full px-6 py-3 text-sm font-bold text-white" style={{ background: "linear-gradient(135deg, " + C.primary + ", " + C.primaryDark + ")" }}>Optimize my profile</Link>
      ...
      <section className="mx-auto max-w-5xl px-6 py-16">
        <div className="rounded-3xl px-8 py-12 text-center" style={{ background: "linear-gradient(135deg, " + C.primary + ", " + C.primaryDark + ")" }}>
          <h2 className="text-2xl font-extrabold text-white md:text-3xl">Ready to stand out to recruiters?</h2>
          <p className="mx-auto mt-3 max-w-xl text-sm text-white/80">Optimize your LinkedIn and your resume together. No credit card needed.</p>
