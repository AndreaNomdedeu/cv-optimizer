...
<div style={{ color: MUT, fontSize: 13, marginTop: 6 }}>{fileName ? (<span onClick={(e) => { e.preventDefault(); e.stopPropagation(); setFileName(''); }} style={{ color: '#B4245F', fontWeight: 600, cursor: 'pointer' }}>Remove file</span>) : 'PDF or DOCX Â· up to 10 MB'}</div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 6 }}>
          <Link href="/dashboard" style={{ color: MUT, fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>â Back to dashboard</Link>
                  <button style={{ padding: '14px 28px', borderRadius: 12, border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 15, color: '#fff', background: running ? '#9aa0b8' : P, opacity: running ? 0.8 : 1 }} onClick={runCorePath} disabled={running}>{running ? ' Analyzing your CV...' : 'Continue to Review â'}</button>
        </div>
      </div>
    </main></div>
  );
