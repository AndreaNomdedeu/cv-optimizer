        ...
        {step === 1 && (
          ...
          {mode === 'upload' ? (
            <label style={{ display: 'block', border: 'dashed 2px ' + (fileName ? '#9AD8B8' : '#C9CCE8'), borderRadius: 14, padding: '40px 20px', textAlign: 'center', cursor: 'pointer', background: fileName ? '#F2FBF6' : '#FCFCFF' }}>
                <input type="file" accept=".pdf,.doc,.docx,.txt,.rtf,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" style={{ display: 'none' }} onChange={(e) => { const f = e.target.files && e.target.files[0]; if (!f) { setFileName(''); return; } const nm = f.name.toLowerCase(); const isText = nm.endsWith('.txt') || nm.endsWith('.rtf') || nm.endsWith('.md'); setFileName(f.name); if (isText) { const rd = new FileReader(); rd.onload = () => setCvText(String(rd.result || '')); rd.readAsText(f); } else { setMode('paste'); setCvText(''); setErr('We received ' + f.name + ', but browsers cannot read the text inside PDF or Word files reliably. Please open your CV, select all the text (Ctrl+A), copy it (Ctrl+C) and paste it in the box below.'); } }} />
