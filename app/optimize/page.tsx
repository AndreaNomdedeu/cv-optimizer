        ...
        {step === 1 && (
          ...
          {mode === 'upload' ? (
            <label style={{ display: 'block', border: 'dashed 2px ' + (fileName ? '#9AD8B8' : '#C9CCE8'), borderRadius: 14, padding: '40px 20px', textAlign: 'center', cursor: 'pointer', background: fileName ? '#F2FBF6' : '#FCFCFF' }}>
                undefined                <input type="file" accept=".pdf,.doc,.docx,.txt,.rtf,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" style={{ display: 'none' }} onChange={async (e) => { const f = e.target.files && e.target.files[0]; if (!f) { setFileName(''); return; } const nm = f.name.toLowerCase(); setFileName(f.name); setErr(''); const isText = nm.endsWith('.txt') || nm.endsWith('.rtf') || nm.endsWith('.md'); if (isText) { const rd = new FileReader(); rd.onload = () => setCvText(String(rd.result || '')); rd.readAsText(f); return; } if (nm.endsWith('.pdf')) { try { setCvText(''); setErr('Reading your PDF...'); const pj = (window as any).pdfjsLib || await new Promise((res, rej) => { const s = document.createElement('script'); s.src = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js'; s.onload = () => res((window as any).pdfjsLib); s.onerror = rej; document.body.appendChild(s); }); pj.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js'; const buf = await f.arrayBuffer(); const pdf = await pj.getDocument({ data: buf }).promise; let txt = ''; for (let p = 1; p <= pdf.numPages; p++) { const page = await pdf.getPage(p); const c = await page.getTextContent(); txt += c.items.map((it: any) => it.str).join(' ') + '\n'; } const clean = txt.replace(/[ \t]+/g, ' ').trim(); if (!clean) { setMode('paste'); setErr('This PDF has no selectable text (it may be a scan). Please paste your CV text below.'); return; } setCvText(clean); setErr(''); } catch (err) { setMode('paste'); setErr('Could not read the PDF automatically. Please open your CV, select all (Ctrl+A), copy (Ctrl+C) and paste it below.'); } return; } setMode('paste'); setCvText(''); setErr('We received ' + f.name + ', but browsers cannot read Word files reliably. Please open your CV, select all the text (Ctrl+A), copy it (Ctrl+C) and paste it in the box below.'); }} />
